 ...
 */
        ...
        public:
            ...
            /**
             * Return whether or not the centroids of the primitives are the same
             * @param prims
             * @param n
             * @param xyz
             * @return true if all centroids have the same XYZ coordinate value
             */
            bool check(const std::vector<Primitive *> &prims, size_t n, int xyz) {
                while (--n > 0 && prims[n]->get_bbox().centroid() == prims[0]->get_bbox().centroid());
                return n == 0;
            }
