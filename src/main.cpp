...
void usage(const char *binaryName) {
    ...
    printf("  -H               Use uniform hemisphere sampling for direct lighting if present\n");
    printf("  -V               Use uniform hemisphere sampling for environment lighting if present\n");
    printf("  -L               Use thin-lens ray generation if present\n");
    printf("  -b  <INT>        Camera lens radius\n");
    printf("  -d  <INT>        Focal distance\n");
...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv, "s:l:t:m:e:h:H:f:r:c:a:p:b:d:L:V:")) != -1) {  // for each option...
        switch (opt) {
            case 'f':
                write_to_file = true;
            ...
            case 'H':
                ...
                break;
            case 'V':
                config.pathtracer_env_hemisphere_sample = true;
                optind--;
                break;
            case 'L':
                config.pathtracer_thin_lens = true;
                optind--;
                break;
            default:
                usage(argv[0]);
