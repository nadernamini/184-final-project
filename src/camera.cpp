...
namespace CGL {
    ...
    Ray Camera::generate_ray(double x, double y) const {
        ...
        //
        double re_x, re_y;
        re_x = ((2.0 * x) - 1.0) * tan(radians(hFov) * .5);
        re_y = ((2.0 * y) - 1.0) * tan(radians(vFov) * .5);
        Vector3D dir = c2w * Vector3D(re_x, re_y, -1.0);
        dir.normalize();
        Ray r = {pos, dir};
        r.min_t = nClip;
        r.max_t = fClip;
    ...
    Ray Camera::generate_ray_for_thin_lens(double x, double y, double rndR, double rndTheta) const {
        ...
        Vector3D pLens(lensRadius * sqrt(rndR) * cos(2 * PI * rndTheta),
                       lensRadius * sqrt(rndR) * sin(2 * PI * rndTheta), 0), pFocus;
        double re_x, re_y, t;
        re_x = ((2.0 * x) - 1.0) * tan(radians(hFov) * .5);
        re_y = ((2.0 * y) - 1.0) * tan(radians(vFov) * .5);
        Vector3D dir = Vector3D(re_x, re_y, -1.0);
        dir.normalize();
        Ray red = {{0, 0, 0}, dir};
        t = (-focalDistance - red.o.z) / red.d.z;
        pFocus = red.o + t * red.d;
        dir = c2w * (pFocus - pLens);
        dir.normalize();
        Ray blue = {c2w * pLens + pos, dir};
        blue.min_t = nClip;
        blue.max_t = fClip;
        return blue;
    }
