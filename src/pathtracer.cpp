...
namespace CGL {
    ...
    PathTracer::PathTracer(size_t ns_aa,
        ...
        this->thin_lens = thin_lens;
        this->filename = filename;

        if (envmap) {
            this->envLight = new EnvironmentLight(envmap, env_hemisphere_sample);
        } else {
            this->envLight = NULL;
        }
    ...
    Spectrum PathTracer::estimate_direct_lighting_hemisphere(const Ray &r, const Intersection &isect) {
        ...
        // (Part 3): sampling loop here
        for (int i = 0; i < num_samples; i++) {
            w_in = hemisphereSampler->get_sample();
            Ray ray(hit_p + (EPS_D * o2w * w_in), o2w * w_in);
            ray.min_t = EPS_D;
            Intersection intersect;
            if (bvh->intersect(ray, &intersect)) {
                L_out += intersect.bsdf->get_emission() * isect.bsdf->f(w_out, w_in) * cos_theta(w_in);
            }
        }
        L_out /= num_samples * pdf;
    ...
    Spectrum PathTracer::estimate_direct_lighting_importance(const Ray &r, const Intersection &isect) {
        ...
        Vector3D w_in;
        double p = 0, o = 0;
        double *distToLight = &o, *pdf = &p;

        // (Part 3): looping over scene lights
        // COMMENT OUT `normal_shading` IN `est_radiance_global_illumination` BEFORE YOU BEGIN
        for (SceneLight *sl: scene->lights) {
            num_samples = sl->is_delta_light() ? 1 : ns_area_light;
            for (int i = 0; i < num_samples; i++) {
                rad = sl->sample_L(hit_p, wi, distToLight, pdf);
                w_in = w2o * *wi;
                wi->normalize();
                if (w_in.z < 0.0)
                    continue;
                Ray ray(hit_p + (EPS_D * *wi), *wi);
                ray.max_t = *distToLight;
                ray.min_t = EPS_D;
                Intersection intersect;
                if (!bvh->intersect(ray, &intersect)) {
                    L_out += *pdf == 0 ? Spectrum{} : rad * cos_theta(w_in) * isect.bsdf->f(w_out, w_in) /
                                                      (*pdf * num_samples);
                }
            }
        }
    ...
    Spectrum PathTracer::at_least_one_bounce_radiance(const Ray &r, const Intersection &isect) {
        ...
        */
        ...
        const double cpdf = (r.depth == max_ray_depth && max_ray_depth > 1) ? 1 : (1. - (1. / max_ray_depth));
        // (Part 4): Here is where your code for sampling the BSDF,
        ...
        if (coin_flip(cpdf)) {
            ...
            if (bvh->intersect(ray, &new_isect)) {
                if (isect.bsdf->is_delta()) {
                    L_rec = at_least_one_bounce_radiance(ray, new_isect) + zero_bounce_radiance(ray, new_isect);
                } else {
                    L_rec = at_least_one_bounce_radiance(ray, new_isect);
                }
            } else if (envLight) { // if there is an environment map
                L_rec = envLight->sample_dir(ray);
            }

            L_out += *pdf == 0 ? Spectrum{} : L_rec * bsdf * abs_cos_theta(*w_in) / *pdf / cpdf;
        }
        return L_out;
    }
    ...
    Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
        ...
        Spectrum s = Spectrum(), tmp;
        double s1 = 0, s2 = 0, tmp_ill;
        int n = 0;
        Ray r(Vector3D(0, 0, 0), Vector3D(0, 0, 0));
        Vector2D sample, rands;
        if (ns_aa == 1) {
            if (thin_lens) {
                rands = gridSampler->get_sample();
                r = camera->generate_ray_for_thin_lens((x + 0.5) / sampleBuffer.w,
                                                       (y + 0.5) / sampleBuffer.h, rands.x, rands.y);
            } else {
                r = camera->generate_ray((x + 0.5) / sampleBuffer.w, (y + 0.5) / sampleBuffer.h);
            }
            ...
            for (int i = 0; i < ns_aa; i++) {
                ...
                if (thin_lens) {
                    rands = gridSampler->get_sample();
                    r = camera->generate_ray_for_thin_lens((x + sample.x) / sampleBuffer.w,
                                                           (y + sample.y) / sampleBuffer.h, rands.x, rands.y);
                } else {
                    r = camera->generate_ray((x + sample.x) / sampleBuffer.w, (y + sample.y) / sampleBuffer.h);
                }
                r.depth = max_ray_depth;
                tmp = est_radiance_global_illumination(r);
                s += tmp;
                n++;
                tmp_ill = tmp.illum();
                s1 += tmp.illum();
                s2 += std::pow(tmp_ill, 2);
                if (n % samplesPerBatch == 0) {
                    if (converged(s1, s2, n)) {
                        break;
                    }
                }
            }
    ...
    bool PathTracer::converged(double s1, double s2, int n) {
        double denom = n == 1 ? 1 : n - 1;
        double mean = s1 / n, var = (1 / denom) * (s2 - (std::pow(s1, 2) / n));
        double I = 1.96 * (std::sqrt(var) / std::sqrt(n));
        return I <= (maxTolerance * mean);
    }
